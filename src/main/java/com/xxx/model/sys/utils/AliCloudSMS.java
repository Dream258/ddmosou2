...
public class AliCloudSMS {
	...
	// 发送短信
	public static SendSmsResponse sendSms(String phone, String code,HttpSession session) throws ClientException {
		// 可自助调整超时时间
	...
	// 发送短信（忘记密码）
	public static SendSmsResponse sendSms2(String phone, String code,HttpSession session) throws ClientException {
		// 可自助调整超时时间
		System.setProperty("sun.net.client.defaultConnectTimeout", "10000");
		System.setProperty("sun.net.client.defaultReadTimeout", "10000");
		// 初始化acsClient,暂不支持region化
		...
		// 必填:短信模板-可在短信控制台中找到
		//request.setTemplateCode("SMS_152130883");
		request.setTemplateCode("SMS_152130879");
		// 可选:模板中的变量替换JSON串,如模板内容为"亲爱的${name},您的验证码为${code}"时,此处的值为
		// request.setTemplateParam("{\"name\":\"Tom\", \"code\":\"123\"}");
		request.setTemplateParam("{\"code\":\"" + code + "\"}");
		System.err.println(request.getTemplateParam());
